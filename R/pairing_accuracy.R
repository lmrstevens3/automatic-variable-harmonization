...
assign_true_pairing <- function(data, ref_ID_col, paired_ID_col, class_cols =  NULL, parallel = F, cores = (parallel::detectCores() - 2)) {
    ...
    data %>% group_by(., across(all_of(class_cols))) %>% 
        {if (parallel) {
...
summarize_actual_pairings <- function(data, ref_ID_col, class_col = NULL, ref_group_col = NULL, multiple_correction = F) {
    ...
    data %>% group_by(across(any_of(class_col))) %>%
        mutate(across(any_of(c(ref_ID_col, ref_group_col)), ~n_unique(.x), .names = "n_{.col}")) %>%
        {if(!length(ref_group_col) == 0) {
            group_by(., across(all_of(c(class_col, ref_group_col)))) %>% 
                mutate(!!as.symbol(n_ref_in_refgroup) := n_unique(!!as.name(ref_ID_col))) %>% 
                group_by(across(any_of(class_col))) %>%
                mutate(across(all_of(n_ref_in_refgroup), ~sum(.x[!duplicated(!!as.name(ref_ID_col))] - 1), .names = "n_same_group_pairs")) %>%
            {if(multiple_correction) {
                mutate(., actual_pairings = !!as.name(paste0("n_", ref_group_col)) - 1,
                          n_actual_pairings = calc_n_pairings(!!as.name(ref_ID_col),!!as.name(ref_group_col)))
            } else {
                mutate(., actual_pairings = !!as.name(paste0("n_", ref_ID_col)) - !!as.name(n_ref_in_refgroup), 
                          n_actual_pairings = calc_n_pairings(!!as.name(ref_ID_col)) - n_same_group_pairs)
            }}
        } else {
            mutate(., actual_pairings = !!as.name(paste0("n_", ref_ID_col)) - 1, 
...
summarize_predictions <- function(data, ref_cols, paired_ID_col, true_pairing_col, rank_col, rank_cutoffs = c(1,5,10), multiple_correction = F, paired_group_col = NULL, class_col = NULL) {
    ...
    data %>% group_by(across(all_of(ref_cols))) %>%
        #calc booleans for valid predictions based on rank cutoff and true predictions
        mutate(across(all_of(rank_col), ~sum(!is.na(.x)), .names = "{.col}_pairs")) %>%
        filter(across(all_of(rank_col), ~(.x <= max(rank_cutoffs) | sum(is.na(.x)) == length(.x)))) %>% 
        ungroup() %>%
        mutate(map_dfc(pred_cuts, ~((!is.na(.y)) & .y <= .x), .y = .[[rank_col]])) %>%
        mutate(map_dfc(select(., all_of(true_pred_cols)), 
                       ~calc_true_positives(.y, .x), .y = .[[true_pairing_col]])) %>%
        #calculate total predictions and true predictions for each refID
        group_by(across(all_of(ref_cols))) %>%
        {if(multiple_correction) {
            mutate(., across(matches("top\\d+_(?:pred|true)"), ~length(unique((!!as.name(paired_group_col))[.x]))))
        } else { 
            mutate(., across(matches("top\\d+_(?:pred|true)"), ~sum(.x, na.rm = T)))
        }} %>% 
        dplyr::select(all_of(c(class_col,ref_cols, paste0(rank_col, "_pairs"))),  matches("_pred$")) %>% 
        unique() %>% 
        #get total counts (n) (by class_col if provided):
        group_by(across(any_of(class_col))) %>% 
        mutate(across(matches("n_|_pred$"), sum, na.rm = T, .names = "n_{.col}")) %>% 
        ungroup()
}
...
calc_accuracy <- function(data, ref_ID_col, paired_ID_col, true_pairing_col, rank_col, rank_cutoffs = c(1,5,10), class_col = NULL, ref_group_col = NULL, multiple_correction = F, paired_group_col = NULL, remove_refID_stats = T) {
    ...
    data <- summarize_predictions(data, c(ref_ID_col, ref_group_col), paired_ID_col, true_pairing_col, rank_col, rank_cutoffs, multiple_correction, paired_group_col, class_col) %>%
        ...
        {if(remove_refID_stats) {
            select(., any_of(class_col), matches("^n_"), -matches("n_.*in_group|same_group_pairs$")) 
        } else {.}} %>%
        ungroup() %>% unique()
    ...
    totals <- {if(is.null(class_col)) NULL 
                      ...
                      summarise(across(matches("^n_"), ~sum(.x[!duplicated(!!as.name(class_col))], na.rm = T)),
    ...
    bind_rows(totals, data) %>%
        mutate(., pmap_dfc(list(true_pred_cols, pred_cols, actual_cols),
                           ~calc_F1(..4[[..1]], ..4[[..2]], ..4[[..3]], gsub("true.*$", "", ..1)), ..4 = .)) %>% 
        select(any_of(c(class_col, ref_ID_col)), everything()) %>% 
        arrange(across(any_of(c(class_col, ref_id_col))))
